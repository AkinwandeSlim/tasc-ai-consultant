...
/**
 * Kinetic Grid — reactive dot grid pulled toward cursor.
 *
 * Canvas-based interactive background: dots at grid intersections
 * are attracted toward the mouse cursor with a spring effect,
 * grid lines brighten near the cursor, and a fading trail line
 * follows the mouse.
 *
 * Adapted from Originkit.
 */
    ...
    const frame = () => {
        ...
        for (let rIdx = 0; rIdx < cols[c].length; rIdx++) {
          ...
          const prox = m.active
            ? Math.max(
                0,
                1 -
                  Math.sqrt((m.x - d.x) ** 2 + (m.y - d.y) ** 2) / R,
              )
            : 0;
          if (right) {
            ctx.globalAlpha = 0.15 + prox * 0.45;
      ...
      for (const d of dots) {
        const prox = m.active
          ...
          : 0;
        ctx.globalAlpha = 0.5 + prox * 0.3;
        ctx.fillStyle = dotColor;
        ctx.beginPath();
        ctx.arc(d.x, d.y, 1.2 + prox * 1.3, 0, 2 * Math.PI);
        ctx.fill();
      }
